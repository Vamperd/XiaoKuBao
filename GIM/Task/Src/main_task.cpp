 ...
 * @LastEditTime: 2025-01-17 15:32:34
...
void RcSetMode() 
{
  if ((rc_ptr->rc_l_switch() == hw_rc::SwitchState::kDown) || (rc_ptr->rc_l_switch() ==hw_rc::SwitchState::kMid)) {
...
void MotorUpdate() 
{
  HW_ASSERT(Roll_motor_ptr != nullptr, "ptr to feed motor is nullptr", Roll_motor_ptr);
  switch (roll_mode) {
      case kRollMode_Fast:
...
void calcMotorInput() 
{
  HW_ASSERT(Roll_motor_ptr != nullptr, "ptr to roll motor is nullptr", Roll_motor_ptr);
...
void setCommData()
{
  HW_ASSERT(Roll_motor_ptr != nullptr, "pointer to roll Motor is nullptr", Roll_motor_ptr);
